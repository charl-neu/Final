...
#define BUILDING_MIN_RELATIVE_HEIGHT     35        // Minimum height in % of the screenHeight //changed for game balance
#define BUILDING_MAX_RELATIVE_HEIGHT     50        // Maximum height in % of the screenHeight //changed for game balance
...
#define MAX_PLAYER_POSITION              45        // Maximum x position % //changed

#define GRAVITY                       30.67f //changed to make gravity more noticable
#define DELTA_FPS                        60
...
//------------------------------------------------------------------------------------
static const int screenWidth = 2480; //change to make a bigger arena
...
static int team1point = 0; //changed to track points
static int team2point = 0; //changed to track points
            ...
            else
            {
                if (UpdateBall(playerTurn))                       // If collision
                {
                    ...
                    else
                    {
                        ...
                        if (leftTeamAlive) //Changed so the winner gains a point
                        {
                            team1point++;
                        }
                        else
...
void DrawGame(void)
{
    ...
    char str[50];  //changes done to display each team's score
    sprintf(str, "p1 score: %d", team1point);
    DrawText(str, 20, 20, 20, BLUE);

    sprintf(str, "p2 score: %d", team2point);
    DrawText(str, screenWidth-150, 20, 20, RED);
